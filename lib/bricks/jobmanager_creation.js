...
class JobManagerCreation extends Brick {
  ...
  process(context) {
    ...
    const jobId = context.data.id;
    const cementHelper = this.cementHelper;
    ...
    Promise.all([createScenarioPromise, instancePromise])
      ...
      .then((execution) => {
        ...
        execution.instances.forEach((instance) => {
          const sentContext = cementHelper.createContext({
            id: execution.id,
          ...
          sentContext.on(EVENTS.DONE, () => {
            ...
            if (counter === 0) {
              ...
              .on(EVENTS.DONE, () => context.emit(EVENTS.DONE))
              .publish();
            }
          });
