...
class JobManagerCancelation extends BaseHelper {
  ...
  _process(context) {
    ...
    this.getExecutionObject(context)
      .then((_execution) => {
    ...
    return executionRest.getExecution(executionId)
      .catch((restErr) => {
        ...
        throw {
          ...
          response: new Error(`Cannot get an execution object: ${restErr.statusCode}`)
        }
      })
  }
  ...
  cancelExecution(execution) {
    ...
    return Promise.all(promises)
      .then((removedArray) => {
        return that.getCancelStatusesResponse(execution, removedArray);
      })
      .catch((err) => {
  ...
  getCancelStatusesResponse(execution, removedArray) {
    return execution.instances.map((instance, index) => {
      return {
        hostName: instance.hostName,
        cancelStatus: removedArray[index] ? 'canceled' : 'inprogress'
      }
    });
  }
  ...
  updateExecutionState(execution) {
    this.logger.info('Update execution state to Canceled:', execution.id);
    const that = this;
    return executionRest.updateExecution(execution.id, { state: 'Canceled' })
      .catch((err) => {
        that.logger.error(`Cannot update execution state: ${err.statusCode}`);
        throw {
          returnCode: EVENTS.ERROR,
          brickName: that.cementHelper.brickName,
          response: new Error(`Cannot update execution state: ${err.statusCode}`)
        }
      });
  }
