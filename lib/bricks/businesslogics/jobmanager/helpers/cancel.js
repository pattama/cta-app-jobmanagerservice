...
class ExecutionCancelation extends BaseHelper {
  ...
  _process(context) {
    ...
    this.cancelExecution(context)
      .then((cancelProgresses) => {
        this.logger.info(`Execution was canceled:${execution.id}`, cancelProgresses);   //TODO - change log level to debug
  ...
  sendCancelStateToEds(executionId, cancelProgresses) {
    this.logger.info('Send canceled state for execution:', executionId);   //TODO - change log level to debug
