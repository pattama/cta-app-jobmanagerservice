...
class JobManagerRun extends BaseHelper {
  constructor(cementHelper, logger) {
    ...
    this.executionRest = executionRest;
    this.instancesRest = instancesRest;
  }
  ...
  createExecution(context) {
    return this.executionRest.createExecution({
  ...
  updateExecution(context, execution, instances) {
    ...
    const commandsCount = this.getCommandsCount(configuration.runMode, instances.length, testsCount);
    return this.executionRest.updateExecution(
